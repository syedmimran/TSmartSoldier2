...
void TSMSoldier::setProfileStanding2(int profile, int bone_num) {
	...
	switch (bone_num) {
		...
		case SMS_LeftHip:
			k = boneRot2[PF_STANDING1][bone_num].x - boneRot2[profile][bone_num].x;
			k1 = boneRot2[PF_STANDING1][bone_num].z - boneRot2[profile][bone_num].z;
			q = quat(boneRot[bone_num].x + stepAngle * 5.0 - 250 * x40 + k*forwardMove, boneRot[bone_num].y - stepAngle * 10.0 * x50, boneRot[bone_num].z + k1 * forwardMove);
			break;
		...
		case SMS_LeftKnee:
			if (stepAngle_off < 0.0)
				f1 = 2.5;
			k   =  boneRot2[PF_STANDING1][bone_num].x - boneRot2[profile][bone_num].x;
			k1  = boneRot2[PF_STANDING1][bone_num].y - boneRot2[profile][bone_num].y;
			zz1 =  boneRot2[PF_STANDING1][bone_num].z - boneRot2[profile][bone_num].z;
			q = quat(boneRot[bone_num].x - stepAngle_off * 3.0 * f1 + 1750 * x75 + 200 * x50 + k * forwardMove, boneRot[bone_num].y + k1 * forwardMove, boneRot[bone_num].z - fabsMovingSpeed + zz1 * forwardMove);
			break;
		...
		case SMS_LeftElbow:
			q = quat(boneRot[bone_num].x, boneRot[bone_num].y, boneRot[bone_num].z - x30 * 1.0 * stepAngle_off * aimValue + 10 * fabsMovingSpeed * aimValue);
			break;
