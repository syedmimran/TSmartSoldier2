...
void TSMSoldier::setProfileStanding1(int profile, int bone_num) {
	...
	switch (bone_num) {
		...
		case SWAT_LeftArm:
			q = quat(boneRot[bone_num].x + fabsMovingSpeed * 5 * aimValue, boneRot[bone_num].y, boneRot[bone_num].z + stepAngle * 7.0 * aimValue);
			break;

		case SWAT_LeftElbow:
			q = quat(boneRot[bone_num].x, boneRot[bone_num].y, boneRot[bone_num].z - x25 * 5.0 * stepAngle_off * aimValue + 10 * fabsMovingSpeed * aimValue);
			break;
		...
		case SWAT_RightArm:
			q = quat(boneRot[bone_num].x + fabsMovingSpeed * 5 * aimValue, boneRot[bone_num].y, boneRot[bone_num].z + stepAngle * 1.5 * aimValue);
			break;
